...
export async function atualizarTarefa(id, updates) {
  ...
  try {
    ...
    // Se houve alteração nos colaboradores, você pode tratar aqui também
    const colaboradoresAntigos = tarefaAtual.colaboradores || [];
    const colaboradoresNovos = updates.colaboradores || colaboradoresAntigos;

    // Lógica para saber se alguém foi removido
    const removidos = colaboradoresAntigos.filter(c => !colaboradoresNovos.includes(c));

    // Chama função de métrica, passando tudo que ela precisa
    await atualizarMetricasTarefa(tarefaAtual, updates, removidos);

    // Filtra campos proibidos
    const dadosFiltrados = Object.fromEntries(
      Object.entries(updates).filter(([chave]) => chave !== "_id" && chave !== "id")
    );
...
export async function atualizarMetricasTarefa(tarefaAtual, updates) {
  try {
    ...
    const tagsNovas = updates.tags || tagsAntigas;
...
export async function deletarTarefa(id) {
  ...
  try {
    ...
    // Corrige o cálculo de tempo
    const ms = tarefa.dataConclusao
      ? new Date(tarefa.dataConclusao) - new Date(tarefa.dataCriacao)
      : null;

    const envolvidos = [
      tarefa.criador,
      ...(Array.isArray(tarefa.colaboradores) ? tarefa.colaboradores : [])
    ];

    for (const userId of envolvidos) {
      await atualizarContadorStatus(userId, tarefa.status, -1);

      const tagsAntigas = tarefa.tags || [];
      const tagsNovas = [];

      await atualizarRankingTags(userId, tagsNovas, tagsAntigas);
      ...
      if (tarefa.dataConclusao) {
        ...
        await atualizarEstatisticasProdutividade(userId, null, false, false, true,tarefa);
      }
    }
