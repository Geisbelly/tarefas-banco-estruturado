...
export async function deletarTarefa(id) {
  ...
  try {
    ...
    if (result.deletedCount === 0) {
      ...
      const ms = new Date(tarefa.dataCriacao) - new Date(tarefa.dataConclusao);
      await atualizarEstatisticasProdutividade(tarefa.criador, ms, true); // decrementa
      await registrarConclusaoPorData(tarefa.criador, tarefaAtual.dataConclusao); // remove de concluídos
