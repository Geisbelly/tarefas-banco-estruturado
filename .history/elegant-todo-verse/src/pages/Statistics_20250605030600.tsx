...
interface ApiCriadasPorDia { // Para a nova API de tarefas criadas por período
    [date: string]: number;
}
// --- Fim Tipos ---
...
const formatDataParaAPI = (data: Date): string => {
  ...
  // Para garantir que estamos formatando a data correta, especialmente se ela já estiver em UTC
...
const App = () => {
  ...
  const [startDateConcluidas, setStartDateConcluidas] = useState(formatDataParaAPI(new Date(seteDiasAtrasDate.toISOString().split('T')[0] + 'T00:00:00Z')));
  const [endDateConcluidas, setEndDateConcluidas] = useState(formatDataParaAPI(new Date(hojeDate.toISOString().split('T')[0] + 'T00:00:00Z')));
  const [startDateCriadas, setStartDateCriadas] = useState(formatDataParaAPI(new Date(seteDiasAtrasDate.toISOString().split('T')[0] + 'T00:00:00Z')));
  const [endDateCriadas, setEndDateCriadas] = useState(formatDataParaAPI(new Date(hojeDate.toISOString().split('T')[0] + 'T00:00:00Z')));
  ...
  useEffect(() => {
    ...
    const dStartConcluidas = new Date(startDateConcluidas + "T00:00:00Z");
    ...
    if (!datesAreValid) {
        ...
        setIsLoading(false); // Permite que a UI seja atualizada com a mensagem de erro
        // Limpa os dados dos gráficos que dependem dessas datas, mas mantém os outros
    ...
    const fetchAllDashboardData = async (email: string, deConcluidas: string, ateConcluidas: string, deCriadas: string, ateCriadas: string) => {
      ...
      try {
        ...
        let currentConcluidaDate = new Date(dStartConcluidas); // Usa o objeto Date validado (já UTC)
        ...
        const processedActivityConcluidas = diasConcluidasParaGrafico.map(dataDia => {
            ...
            const contagem = concluidasPorDiaApiData[dataFormatadaChave] ?? 0;
             console.log(`[Concluídas] Data: ${dataFormatadaChave}, Label: ${diaLabel}, Contagem: ${contagem}`);
        ...
        try {
            ...
            if (criadasResponse.ok) {
                ...
                let currentCriadaDate = new Date(dStartCriadas); // Usa o objeto Date validado (já UTC)
                ...
                const processedActivityCriadas = diasCriadasParaGraficoList.map(dataDia => {
                    ...
                    console.log(`[Criadas] Data: ${dataFormatadaChave}, Label: ${diaLabel}, Contagem: ${contagem}`);
