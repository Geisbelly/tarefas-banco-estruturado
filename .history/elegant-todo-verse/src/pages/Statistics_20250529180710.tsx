...
const CustomChartTooltipContent = (props: TooltipProps<any, any>) => {
    ...
    return (
      <div className="p-2 text-xs bg-gray-800/90 border border-gray-700 rounded-lg shadow-xl text-gray-200">
        ...
        {payload.map((entry, index: number) => (
          <p key={`item-${index}`} style={{ color: (entry as any).color || (entry as any).fill }} className="capitalize">
...
const mockApiUser = { email: "utilizador@exemplo.com", name: "Utilizador Teste" }; 

// Simula a estrutura do seu serviço getTask
const getTask = async (email: string): Promise<Task[]> => {
  console.log(`A obter tarefas para o email: ${email} da API MOCK`);
  // Simula atraso da API
  await new Promise(resolve => setTimeout(resolve, 1000));

  // Tarefas de exemplo - garanta que esta estrutura corresponde à resposta da sua API
  // E que dataCriacao é uma string de data válida para `new Date()`
  const sampleTasksFromApi: Task[] = [
    { id: "api-1", nome: "API: Rever Contrato", status: "pendente", dataCriacao: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(), tags: ["api-trabalho", "legal"], comentarios: [{texto: "Verificar cláusula X", data: new Date().toISOString(), autorId: "api-user-1"}]},
    ...
    { id: "api-3", nome: "API: Comprar Bilhetes", status: "concluida", dataCriacao: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(), dataConclusao: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(), tags: ["api-pessoal", "viagem"], comentarios: [] },
    { id: "api-4", nome: "API: Agendar Reunião Cliente", status: "pendente", dataCriacao: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000).toISOString(), tags: ["api-trabalho", "cliente"], comentarios: [] },
    { id: "api-5", nome: "API: Escrever Artigo Blog", status: "em andamento", dataCriacao: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(), tags: ["marketing", "conteúdo"], comentarios: [{texto: "Precisa de imagens", data: new Date().toISOString(), autorId: "api-user-2"}] },
