...
const Statistics = () => {
  ...
  // Simple StatBox component definition
  const StatBox = ({ label, value, color }: { label: string; value: number; color: string }) => (
    <div className={`rounded p-4 bg-${color}-100 text-${color}-800 flex flex-col items-center`}>
      <span className="text-lg font-semibold">{label}</span>
      <span className="text-2xl font-bold">{value}</span>
    </div>
  );

  // Example counts for demonstration (replace with your stats object)
  const count = {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Tarefas Totais</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-3 gap-4">
            <StatBox label="Pendentes" value={count.pending} color="yellow" />
            <StatBox label="Em Progresso" value={count.inProgress} color="blue" />
            <StatBox label="Concluídas" value={count.done} color="green" />
          </div>
        </CardContent>
      </Card>
    </div>
