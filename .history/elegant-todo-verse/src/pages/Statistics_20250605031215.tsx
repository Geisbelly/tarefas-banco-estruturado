  ...
  if (active && payload && payload.length) {
    return (
      <div className="p-2 text-xs bg-gray-800/90 border border-gray-700 rounded-lg shadow-xl text-gray-200">
        ...
        {payload.map((entry: import('recharts').TooltipPayload, index: number) => (
    ...
    <div className="container mx-auto px-2 sm:px-4 py-8">
      ...
      <Tabs defaultValue="status" className="w-full">
            ...
            <CardContent className="pt-0">
              ...
              {activityCriadasChartData ? (
                ...
                <div className="text-center text-gray-400 py-10">
                  ...
                  <p className="text-xs mt-2">**Ação Necessária:** Verifique se a API <code>/tarefas/criadasPorPeriodo</code> está implementada no seu backend.</p>
                  <p className="text-xs mt-1">Ela deve aceitar <code>userId</code>, <code>de</code>, <code>ate</code> e retornar <code>{"{YYYY-MM-DD: count}"}</code>.</p>
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <TabsContent value="tags">
            ...
            <CardContent className="pt-0">
              {/* GRÁFICO DE TAGS ALTERADO PARA BARRAS VERTICAIS (COLUNAS) */}
              <div className="h-[400px] sm:h-[480px] w-full mb-6"> {/* Aumentei um pouco a altura */}
                <ChartContainer>
                  <BarChart data={tagsChartData} 
                    margin={{ top: 20, right: 20, left: 0, bottom: 70 }} // Aumentar bottom margin para labels rotacionados
                    barCategoryGap="20%"
                  >
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.1)" />
                    <XAxis 
                        ...
                        angle={-45} // Rotacionar labels
                        textAnchor="end" // Ancorar no final para melhor ajuste
                        height={60} // Aumentar altura do eixo X
                    />
                    <YAxis type="number" stroke="#9ca3af" fontSize="10px" allowDecimals={false} />
                    <Tooltip content={<CustomChartTooltipContent />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }}/>
                    <Bar dataKey="count" name="Nº de Tarefas" radius={[4, 4, 0, 0]} > {/* Barras verticais */}
                      {tagsChartData.map((entry, index) => {
                          ...
                          // Usar entry.rank para as cores do pódio
...
const App = () => {
  ...
  seteDiasAtrasDate.setUTCDate(hojeDate.getUTCDate() - 6); // Use UTC for date calculations
  ...
  useEffect(() => {
    ...
    if (!emailToUse) {
      const defaultMockUser = { email: "geisbelly19@gmail.com" }; // Configure seu email mock aqui
      try {
    ...
    // Validação de datas antes de fazer o fetch
    // As datas dos inputs são YYYY-MM-DD. Para comparar, convertemos para objetos Date (meia-noite UTC)
    ...
    const dStartCriadas = new Date(startDateCriadas + "T00:00:00.000Z");
    const dEndCriadas = new Date(endDateCriadas + "T00:00:00.000Z");
    ...
    if (isNaN(dStartConcluidas.getTime()) || isNaN(dEndConcluidas.getTime()) || isNaN(dStartCriadas.getTime()) || isNaN(dEndCriadas.getTime())) {
        datesAreValid = false;
        dateErrorMessage = "Uma ou mais datas do filtro são inválidas.";
        ...
        datesAreValid = false;
        dateErrorMessage = "Filtro 'Criadas': data de início não pode ser maior que a data de fim.";
    }
    ...
    const fetchAllDashboardData = async (email: string, deConcluidas: string, ateConcluidas: string, deCriadas: string, ateCriadas: string) => {
      ...
      try {
        ...
        const completed = statusApiData.concluida ?? 0; // Total concluídas conforme status atual
        setStatusChartData([
        ...
        let currentConcluidaDate = new Date(dStartConcluidas); // Usa objeto Date UTC
        ...
        try {
            console.log("[fetchAllDashboardData] Tentando buscar Tarefas Criadas:", criadasNoPeriodoUrl);
            ...
            if (criadasResponse.ok) {
                ...
                const currentCriadaDate = new Date(dStartCriadas); // Usa objeto Date UTC
            ...
            console.error("[fetchAllDashboardData] Erro API tarefas criadas (verifique se o endpoint existe e está correto):", errorCriadas);
            setActivityCriadasChartData(null);
        }

        const totalTasksAtivas = pending + inProgress; // Corrigido
        const hojeFormatado = formatDataParaAPI(new Date(new Date().toISOString().split('T')[0] + "T00:00:00Z")); // Hoje em UTC para chave
        ...
        if (productivityApiData.taxaConclusaoSemanal === undefined) {
            console.warn("[fetchAllDashboardData] API Produtividade não retornou 'taxaConclusaoSemanal'. Verifique o backend. Usando 0% para o card.");
        }
        ...
        const finalGeneralStats = {
          ...
          uniqueTags: processedTags.length, // Número de tags no top 10
