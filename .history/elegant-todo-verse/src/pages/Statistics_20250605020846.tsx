...
// --- Tipos ---
// Interface Task (se a API /concluidas retornar objetos Task com dataConclusao)
interface Task {
  id: string;
  nome: string;
  // ... outros campos da sua tarefa ...
  dataCriacao: string; // Mantido para referência, mas não usado diretamente no gráfico de concluídas
  dataConclusao?: string | null; // ESSENCIAL para o gráfico de concluídas
  tags?: string[];
  comentarios?: any[]; // Ajuste se necessário
}
...
// --- Componente Statistics (definição permanece a mesma da versão anterior) ---
interface StatisticsProps {
  ...
  activityChartData: { dia: string; tarefasConcluidas: number }[]; // Continua tarefasConcluidas
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllDashboardData = async (email: string) => {
      ...
      // Para a API /concluidas (para o gráfico de atividade dos últimos 7 dias)
      const hoje = new Date();
      ...
      const formatDataParaAPI = (data: Date): string => {
        ...
        const mes = (data.getMonth() + 1).toString().padStart(2, '0'); // Mês é 0-indexado
      ...
      console.log("  Concluídas no Período (para gráfico):", concluidasNoPeriodoUrl);
      ...
      try {
        const responses = await Promise.all([
          ...
          fetch(statusUrl),
          fetch(tagsUrl),
          fetch(concluidasNoPeriodoUrl) // Chamada para a API /concluidas
        ]);
        ...
        const tarefasConcluidasNoPeriodo: Task[] = await concluidasNoPeriodoRes.json(); // Espera-se um array de Tasks
        ...
        console.log("  Tarefas Concluídas no Período (bruto):", JSON.stringify(tarefasConcluidasNoPeriodo));
        ...
        //    A partir da lista de `tarefasConcluidasNoPeriodo`
        const activityMap = new Map<string, number>();
        const diasParaGrafico = Array.from({ length: 7 }, (_, i) => {
            const data = new Date();
            data.setDate(hoje.getDate() - i); // Subtrai dias de hoje
            return data;
        }).reverse(); // Ordena do mais antigo para o mais novo

        const diasFormatadosLabels = diasParaGrafico.map(d => d.toLocaleDateString('pt-BR', { weekday: 'short' }).replace('.', ''));
        diasFormatadosLabels.forEach(label => activityMap.set(label, 0)); // Inicializa o mapa com 0 para cada dia

        (tarefasConcluidasNoPeriodo || []).forEach(task => {
          if (task.dataConclusao) { // **ESSENCIAL: SUA TASK PRECISA TER O CAMPO dataConclusao**
            try {
