...
interface ApiProductivityData {
  totalTarefas?: number;      // Ex: "totalTarefas": 50
  totalComentarios?: number;  // Ex: "totalComentarios": 120
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (storedUserString) {
        ...
        emailForApi = defaultMockUser.email; // Fallback for SSR or non-browser environments
      }
    } catch (e) {
    ...
    if (!emailForApi) {
        ...
        setInitializationError("Email do utilizador não encontrado. Verifique o localStorage ou a configuração.");
        setIsLoading(false);
        return;
    }
    ...
    const fetchAllDashboardData = async (email: string) => {
      ...
      try {
        const [productivityRes, statusRes, tagsRes] = await Promise.all([
        ...
        if (!productivityRes.ok) throw new Error(`Erro Produtividade (${productivityRes.status}): ${await productivityRes.text()}`);
        if (!statusRes.ok) throw new Error(`Erro Status (${statusRes.status}): ${await statusRes.text()}`);
        if (!tagsRes.ok) throw new Error(`Erro Tags (${tagsRes.status}): ${await tagsRes.text()}`);
        if (!allTasksRes.ok) throw new Error(`Erro Todas Tarefas (${allTasksRes.status}): ${await allTasksRes.text()}`);
        ...
        const totalTasks = totalTasksFromProd !== undefined && totalTasksFromProd >= totalTasksFromStatus ? totalTasksFromProd : totalTasksFromStatus;
        ...
        // Se a API /tags retorna todas as tags únicas, use seu tamanho. Senão, calcule de allTasks.
        const uniqueTagsFromApi = new Set((tagsApiData || []).map(t => t.tag)).size;
        const uniqueTagsFromAllTasks = new Set((allTasks || []).flatMap(task => task.tags || [])).size;
        // Decida qual fonte é mais confiável para uniqueTags. Se tagsApiData for apenas o top 5, não serve.
        // Se a API /tags já for exaustiva (todas as tags únicas), use uniqueTagsFromApi.
        // Se /tags só traz o top X, então uniqueTagsFromAllTasks é melhor, assumindo que allTasks contém todas as tags.
        const uniqueTags = uniqueTagsFromAllTasks; // Ou ajuste conforme a lógica da sua API de tags.
        ...
        setInitializationError((error as Error).message || "Ocorreu um erro ao carregar os dados.");
   ...
   if (initializationError) {
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-gray-900 text-white p-4 text-center">
                <ListChecks className="w-16 h-16 mb-4 text-red-500" />
                ...
                {userEmail && <p className="text-sm text-gray-500 mt-2">Utilizador: <code className="bg-gray-700 p-1 rounded">{userEmail}</code></p>}
                 <p className="text-xs text-gray-600 mt-4">Tente recarregar a página ou verifique a consola para mais detalhes.</p>
            </div>
        );
    }
