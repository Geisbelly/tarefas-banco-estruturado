...
interface StatisticsProps {
  ...
  tagsChartData: { tag: string; count: number; rank: number }[]; // Adicionado rank
    ...
    <div className="container mx-auto px-2 sm:px-4 py-8">
      ...
      <Tabs defaultValue="status" className="w-full">
        ...
        <TabsContent value="tags" className="mt-0">
            ...
            <CardContent>
              <div className="h-[400px] sm:h-[450px] w-full mb-6"> {/* Aumentei a altura para 10 tags */}
                <ChartContainer>
                  <BarChart data={tagsChartData} layout="vertical" margin={{ top: 5, right: typeof window !== 'undefined' && window.innerWidth < 640 ? 20 : 30, left: typeof window !== 'undefined' && window.innerWidth < 640 ? 20 : 70, bottom: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.1)" />
                    ...
                    <Bar dataKey="count" name="Nº de Tarefas" radius={[0, 4, 4, 0]} barSize={typeof window !== 'undefined' && window.innerWidth < 640 ? 12 : 18}>
                      {tagsChartData.map((entry, index) => {
                          let fillColor = BAR_CHART_TAG_COLORS_DEFAULT[ (index - PODIUM_COLORS.length) % BAR_CHART_TAG_COLORS_DEFAULT.length];
                          if (index < PODIUM_COLORS.length) {
                            fillColor = PODIUM_COLORS[index];
                          }
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {tagsChartData.map((tagData) => (
                  <Badge 
                    key={tagData.tag} 
                    style={{ 
                        borderLeftColor: tagData.rank <= PODIUM_COLORS.length ? PODIUM_COLORS[tagData.rank -1] : BAR_CHART_TAG_COLORS_DEFAULT[(tagData.rank - 1 - PODIUM_COLORS.length) % BAR_CHART_TAG_COLORS_DEFAULT.length], 
                        borderLeftWidth: 3,
                        order: tagData.rank // Para tentar ordenar visualmente se css permitir (flex/grid)
                    }}
                  >
...
const formatDataParaAPI = (data: Date): string => {
  if (!(data instanceof Date) || isNaN(data.getTime())) {
    console.warn("formatDataParaAPI recebeu data inválida:", data);
    // Retorna uma data padrão ou lança um erro, dependendo da sua preferência de tratamento
...
const App = () => {
  ...
  useEffect(() => {
    ...
    console.log("[App useEffect] Disparado. Email:", emailToUse, "Concluídas:", startDateConcluidas, "-", endDateConcluidas, "Criadas:", startDateCriadas, "-", endDateCriadas);

    // Validação das datas
    const dStartConcluidas = new Date(startDateConcluidas);
    const dEndConcluidas = new Date(endDateConcluidas);
    const dStartCriadas = new Date(startDateCriadas);
    const dEndCriadas = new Date(endDateCriadas);

    if (isNaN(dStartConcluidas.getTime()) || isNaN(dEndConcluidas.getTime()) ||
        isNaN(dStartCriadas.getTime()) || isNaN(dEndCriadas.getTime())) {
        console.warn("[App useEffect] Datas de filtro inválidas. Fetch abortado.");
        setInitializationError("Por favor, insira datas válidas nos filtros.");
        setIsLoading(false);
        setActivityConcluidasChartData([]);
        setActivityCriadasChartData(null);
        // Não limpar generalStats, statusChartData, tagsChartData aqui, pois eles não dependem do filtro de data diretamente
        return;
    }
    if (dStartConcluidas > dEndConcluidas) {
        setInitializationError("Filtro 'Concluídas': data de início após data de fim.");
        setIsLoading(false); setActivityConcluidasChartData([]); return;
    }
    if (dStartCriadas > dEndCriadas) {
        setInitializationError("Filtro 'Criadas': data de início após data de fim.");
        setIsLoading(false); setActivityCriadasChartData(null); return;
    }
    ...
    const fetchAllDashboardData = async (email: string, deConcluidas: string, ateConcluidas: string, deCriadas: string, ateCriadas: string) => {
      setIsLoading(true);
      setInitializationError(null); // Limpa erro anterior ao tentar novo fetch
      ...
      try {
        ...
        const processedTags = (tagsApiData || [])
            .map((item, index) => ({ 
                ...
                rank: index + 1 // Adiciona o rank aqui
            }))
            .filter(item => item.count > 0).sort((a, b) => b.count - a.count).slice(0, 10); // Top 10
        setTagsChartData(processedTags);

        // Gráfico de Concluídas
        const rangeConcluidasStartDt = new Date(deConcluidas + "T00:00:00Z"); // Usar Z para UTC e evitar problemas de fuso na iteração
        const rangeConcluidasEndDt = new Date(ateConcluidas + "T00:00:00Z");
        let diasConcluidasParaGrafico: Date[] = [];
        let currentDate = new Date(rangeConcluidasStartDt);
        while(currentDate <= rangeConcluidasEndDt){
            diasConcluidasParaGrafico.push(new Date(currentDate));
        ...
        const processedActivityConcluidas = diasConcluidasParaGrafico.map(dataDia => {
            ...
            const dataFormatadaChave = formatDataParaAPI(dataDia); // Usa a data UTC para chave
        ...
        try {
            ...
            if (criadasResponse.ok) {
                ...
                const rangeCriadasStartDt = new Date(deCriadas + "T00:00:00Z");
                const rangeCriadasEndDt = new Date(ateCriadas + "T00:00:00Z");
                let diasCriadasParaGraficoList: Date[] = [];
                let currentCriadaDate = new Date(rangeCriadasStartDt);
                 while(currentCriadaDate <= rangeCriadasEndDt){
                ...
                console.warn(`[fetchAllDashboardData] API Tarefas Criadas (${criadasResponse.status}): ${await criadasResponse.text()}.`);
                setActivityCriadasChartData(null);
            }
        } catch (errorCriadas) {
            console.error("[fetchAllDashboardData] Erro API tarefas criadas (pode não existir):", errorCriadas);
            setActivityCriadasChartData(null);
        }
  ...
  if (isLoading) { /* ... (tela de loading como antes) ... */ 
    return (
        <div className="flex flex-col items-center justify-center h-screen bg-gray-900 text-white p-4">
            <Clock className="w-16 h-16 mb-4 text-blue-400 animate-spin" />
            <h2 className="text-2xl font-semibold mb-2">Carregando dados...</h2>
  ...
  if (initializationError) { /* ... (tela de erro como antes) ... */ 
    return (
